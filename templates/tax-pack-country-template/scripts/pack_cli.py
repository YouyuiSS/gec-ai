...
DEFAULT_EXTRACTOR = "xx-example-profile"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a country tax pack via tax-parser-core.")
    ...
    test_parser.add_argument("--extractor", default=DEFAULT_EXTRACTOR)
    ...
    gate.add_argument("--extractor", default=DEFAULT_EXTRACTOR)
...
def core_script(name: str) -> Path:
    ...
    if not path.exists():
        raise SystemExit(
            f"Required core script not found: {path}. "
            "Copy this template under skills/ and install tax-parser-core alongside the real pack."
        )
    return path
